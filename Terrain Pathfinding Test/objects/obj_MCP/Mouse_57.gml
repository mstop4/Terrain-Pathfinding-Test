...
while (!ds_queue_empty(_selected_queue)) {
	...
	if (_tile_index != 1) {
		...
		if (_result) {
			var _x = _cur_unit.x div CELL_SIZE;
			var _y = _cur_unit.y div CELL_SIZE;
			
			occupied_grid[# _x, _y] = 0;
			var _existing_unit = occupied_grid[# _cur_tile_dec[0], _cur_tile_dec[1]];
			...
			if (_existing_unit != 0) {
				with (_existing_unit) {
					...
					var result = find_settle_tile(other.grid_width,other.grid_height,my_settle_grid,other.occupied_grid,settle_process_queue,other.ter_tilemap);
			
					if (result[0] != -1) {
						other.occupied_grid[# result[0], result[1]] = id;
						build_goto_path(result[0] * CELL_SIZE + CELL_SIZE / 2, result[1] * CELL_SIZE + CELL_SIZE / 2);
					}
				}
			}
		
			occupied_grid[# _cur_tile_dec[0], _cur_tile_dec[1]] = _cur_unit;
		}
	}
